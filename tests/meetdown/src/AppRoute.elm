module AppRoute exposing (Route(..), Token(..), decode, encode, encodeWithToken)

import Group exposing (EventId)
import GroupName exposing (GroupName)
import Identifier exposing (DeleteUserToken, GroupId, Id, LoginToken, UserId)
import UserName exposing (Name)
        ...
        , Url.Parser.s "group"
            ...
            |> Url.Parser.map
                (\groupIdSegment groupNameSegment ->
                    case ( Identifier.cryptoHashFromString groupIdSegment, decodeGroupName groupNameSegment ) of
        ...
        , Url.Parser.s "user"
            ...
            |> Url.Parser.map
                (\userIdSegment userNameSegment ->
                    case ( Identifier.cryptoHashFromString userIdSegment, decodeName userNameSegment ) of
...
decodeToken =
    Url.Parser.Query.map4
        ...
        (Url.Parser.Query.string loginTokenName |> Url.Parser.Query.map (Maybe.andThen Identifier.cryptoHashFromString))
        (Url.Parser.Query.string groupIdName |> Url.Parser.Query.map (Maybe.andThen Identifier.cryptoHashFromString))
        (Url.Parser.Query.int eventIdName |> Url.Parser.Query.map (Maybe.map Group.eventIdFromInt))
        (Url.Parser.Query.string deleteUserTokenName |> Url.Parser.Query.map (Maybe.andThen Identifier.cryptoHashFromString))
...
encodeName =
    UserName.toString >> String.replace " " "-" >> Url.percentEncode
...
decodeName text =
    case Url.percentDecode text of
        Just decoded ->
            String.replace "-" " " decoded |> UserName.fromString |> Result.toMaybe
...
encodeWithToken route token =
    Url.Builder.absolute
        (case route of
            ...
            GroupRoute groupId groupName ->
                [ "group", Identifier.cryptoHashToString groupId, encodeGroupName groupName ]
            ...
            UserRoute userId name ->
                [ "user", Identifier.cryptoHashToString userId, encodeName name ]
        ...
        (case token of
            LoginToken loginToken maybeJoinEvent ->
                Url.Builder.string loginTokenName (Identifier.cryptoHashToString loginToken)
                    :: (case maybeJoinEvent of
                            Just ( groupId, eventId ) ->
                                [ Url.Builder.string groupIdName (Identifier.cryptoHashToString groupId)
            ...
            DeleteUserToken deleteUserToken ->
                [ Identifier.cryptoHashToString deleteUserToken |> Url.Builder.string deleteUserTokenName ]
